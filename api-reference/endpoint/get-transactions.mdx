...
The returned `transactionId`, `sourceTxHash`, and `destTxHash` can be used as lookup keys with [Get a tracked transaction](/api-reference/endpoint/get-transaction) and [Get a tracked transaction status](/api-reference/endpoint/get-transaction-status). `destTxHash` is `null` until destination-side data is known.
