...
description: Rate limits vary by authentication tier and endpoint scope. A valid x-api-key selects the authenticated limit tier.
...
`GET /v1/transactions`, `GET /v1/transactions/{txHash}`, and `GET /v1/transactions/{txHash}/status` require a valid `x-api-key`. The key scopes returned tracking data to its associated integrator.
