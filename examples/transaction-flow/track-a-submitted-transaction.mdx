...
3. Call [`GET /v1/transactions/{lookupKey}/status`](/api-reference/endpoint/get-transaction-status) with the source hash or Delora `transactionId`.
...
export async function getTrackingStatus(lookupKey: string): Promise<PollResult> {
  const response = await fetch(
    `https://api.delora.build/v1/transactions/${encodeURIComponent(lookupKey)}/status`,
    {
...
Use [`GET /v1/transactions/{lookupKey}`](/api-reference/endpoint/get-transaction) when the UI or reconciliation process needs the complete record. This sanitized example shows a provider-specific destination chain ID (`57`) normalized to Arbitrum (`42161`).
