...
> – `amount` is always specified in **smallest units** (token decimals applied).
...
| `calldata`     | object[]  | ✅ | Array of transaction objects suitable for a wallet/provider.    |
| `fees`         | object[]  |  | Fees included in the transfer.                                  |

> **Note:** This is a **quote** (estimate), not a guaranteed execution result. Final settlement can differ due to on-chain state at broadcast time. Use appropriate constraints if included in the returned `calldata`.
...
```http
GET https://api.blackhole.build/v1/quotes
  ?originChainId=1
