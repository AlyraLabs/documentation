...
The widget's **Auto** slippage option sends `slippageMode=auto` to Delora instead of a numeric `slippage` value. A configured numeric `slippage`, a numeric preset, or a custom value sends numeric `slippage` instead.
