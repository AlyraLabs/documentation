...
Alyra API provides two ways to request execution paths: **`/quote`** and **`/routes`**. Both endpoints use the same routing engine, but they serve different needs depending on how much control and detail an integrator requires.
...
- **`With /quote`**:\
  User wants to send USDC from Solana and receive DAI on Base. Alyra may internally build a route = _USDC (Solana) → Bridge → USDC (Base) → Swap → DAI (Base)_.\
